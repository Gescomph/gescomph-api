...
public class FormServiceTests
{
    ...
    [Fact]
    public async Task CreateThrowsWhenActiveDuplicateExists()
    {
        ...
        _repo.Setup(r => r.GetAllQueryable())
             .Returns(new List<Form> { new() { Id = 1, Name = "Menu", IsDeleted = false } }.AsQueryable());

        await Assert.ThrowsAsync<BusinessException>(() => _service.CreateAsync(dto));
    }

    [Fact]
    public async Task CreateReactivatesWhenInactiveDuplicateExists()
    {
        var dto = new FormCreateDto { Name = "Menu", Description = "D" };
        ...
        _mapper.Setup(m => m.Map<Form>(dto)).Returns(candidate);
        _repo.Setup(r => r.GetAllQueryable())
             .Returns(new List<Form> { new() { Id = 10, Name = "Menu", IsDeleted = true } }.AsQueryable());

        _repo.Setup(r => r.UpdateAsync(It.Is<Form>(f => f.Id == 10)))
             .ReturnsAsync(new Form { Id = 10, Name = "Menu" });

        _mapper.Setup(m => m.Map<FormSelectDto>(It.IsAny<Form>()))
               .Returns(new FormSelectDto { Id = 10, Name = "Menu" });
