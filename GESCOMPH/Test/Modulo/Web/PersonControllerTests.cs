...
public class PersonControllerTests
{
    ...
    [Fact]
    public async Task PutOkCallsService()
    {
    ...
    public async Task GetReturnsOk()
    {
        _service.Setup(s => s.GetAllAsync())
            .ReturnsAsync(new List<PersonSelectDto>
            {
                new() { Id = 1, FirstName = "A", LastName = "B", CityId = 1, CityName = "C" }
            });
    ...
    {
        var created = new PersonSelectDto
        {
            Id = 2,
            FirstName = "A",
            LastName = "B",
        ...
        var res = await Create().Post(
            new PersonDto { FirstName = "A", LastName = "B", Document = "1", CityId = 1, Address = "X", Phone = "Y" }
        );
