...
namespace Business.Services.SecurityAuthentication
{
    public class RolUserService : BusinessGeneric<RolUserSelectDto, RolUserCreateDto, RolUserUpdateDto, RolUser>, IRolUserService
    {
        ...
        public async Task<RolUser> AsignateRolDefaultAsync(User user)
        {
            if (user == null)
                throw new BusinessException("Usuario requerido para asignar rol por defecto.");

            return await _repository.AsignateRolDefault(user);
        }
