...
namespace Business.Services.SecurityAuthentication
{
        ...
        : BusinessGeneric<UserSelectDto, UserCreateDto, UserUpdateDto, User>, IUserService
    {
        ...
        // ======================================================
        // Eliminar logicamente user + person
        // ======================================================
        public async Task SoftDeleteUserAndPersonAsync(int userId)
        {
            // 1️⃣ Obtener usuario
            var user = await _userRepository.GetByIdAsync(userId)
                       ?? throw new BusinessException("Usuario no encontrado.");
            ...
            // 2️⃣ Soft-delete usuario
            await _userRepository.DeleteLogicAsync(userId);

            // 3️⃣ Soft-delete persona
            await _personRepository.DeleteLogicAsync(personId);
        }
