...
namespace Business.Services.Business
{
        ...
        IEstablishmentService
    {
        ...
        public async Task<IReadOnlyList<EstablishmentCardDto>> GetCardsByPlazaAsync(int plazaId, bool activeOnly = false)
        {
            if (plazaId <= 0) return Array.Empty<EstablishmentCardDto>();

            var filter = activeOnly ? ActivityFilter.ActiveOnly : ActivityFilter.Any;

            var list = await _repo.GetCardsByPlazaAsync(plazaId, filter);
